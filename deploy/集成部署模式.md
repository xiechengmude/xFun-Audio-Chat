...
> **版本**: v1.5-20260130
> **架构**: S2S (原生) + TTS (CosyVoice3 独立服务) + ASR (funasr)
> **验证**: 2026-01-30 RunPod A40 48GB 三服务全量部署实测通过
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.5 | 2026-01-30 | **三服务全量实测**: 修复 ASR funasr 注册、TTS speaker 注入和 generator 逻辑；添加已知问题、链路测试数据、一键部署命令 |
| v1.4 | 2026-01-08 | **实测验证**: RunPod A40 三服务部署成功，添加快速部署指南 |
...
> 以下命令已在 RunPod A40 48GB 上验证通过 (2026-01-30)

### 方式一：一键自动部署 (推荐)

```bash
# 从本地执行，自动创建 Pod + 安装环境 + 下载模型 + 启动 S2S/ASR
# 需要 .env 中配置 RUNPOD_KEY
...
> 注意：auto_deploy.py 自动启动 S2S + ASR。独立 TTS 服务需 SSH 进 Pod 后手动启动（见下方）。
...
pip install torch==2.8.0 torchaudio==2.8.0 torchvision==0.23.0 \
    --index-url https://download.pytorch.org/whl/cu128
pip install sphn aiohttp funasr
pip install -r requirements.txt
pip install 'ruamel.yaml<0.18' --force-reinstall
```
...
# 3. TTS 独立服务 (端口 8004，不加 --use-vllm)
nohup python3 -m web_demo.server.tts_server \
    --model-path pretrained_models/Fun-CosyVoice3-0.5B-2512 \
    --port 8004 --device cuda:0 --host 0.0.0.0 \
    > tts_server.log 2>&1 &
```
...
# ASR 转写测试
curl -X POST http://localhost:8003/api/transcribe \
    -F "audio=@pretrained_models/Fun-ASR-Nano-2512/example/zh.mp3" \
    -F "language=中文"

# TTS 合成测试
curl -X POST http://localhost:8004/api/synthesize \
    -H "Content-Type: application/json" \
    -d '{"text": "你好，测试语音合成。", "speaker_id": "中文女"}'

# 检查端口
ss -tlnp | grep -E '8002|8003|8004'
```

### 实测端口映射 (RunPod 2026-01-30)
...
```bash
# 替换为实际 IP 和端口
curl http://<IP>:22154/health                    # ASR
curl http://<IP>:22155/health                    # TTS
ssh root@<IP> -p 22150 -i ~/.ssh/id_ed25519     # SSH
```
...
| 测试项 | 端点 | 结果 | 耗时 |
|--------|------|------|------|
| S2S WebSocket 连接 | `ws://host:8002/api/chat` | 握手成功 (3 bytes) | 0.77s |
| S2S 对话 (2秒音频) | WebSocket 二进制协议 | 回复: "嗨，你好呀！有什么我可以帮您的吗？" | 首文本 16.7s, 首音频 20.9s |
| ASR 转写 | `POST /api/transcribe` | "开饭时间早上九点至下午五点。" | ~3s (含网络) |
| TTS 合成 (中文女) | `POST /api/synthesize` | 2.56s WAV (122KB) | 4.2s |
| TTS 合成 (中文男) | `POST /api/synthesize/file` | WAV 文件下载 (746KB) | 11.1s |

> S2S 延迟偏高与测试输入为纯正弦波有关，真实语音延迟会更低。

---

## 已知问题与修复记录

### 1. ASR: funasr FunASRNano 未注册

**现象**: `AssertionError: FunASRNano is not registered`

**原因**: funasr 1.3.1 的 `fun_asr_nano/model.py` 内部使用裸 import `from ctc import CTC`，而 `ctc.py` 在同目录下但未加入 Python path，导致模块注册失败。

**修复** (已合入 `asr_server.py`):
```python
import funasr
...
import funasr.models.fun_asr_nano.model  # 显式触发 @tables.register
```

### 2. TTS: CosyVoice3 speaker embeddings 为空

**现象**: `KeyError: '中文女'` — `inference_sft` 调用时 `frontend.spk2info` 为空字典。

**原因**: CosyVoice3 模型本身不包含预置 speaker embeddings，需要从项目的 `utils/new_spk2info.pt` 手动加载并注入。

**修复** (已合入 `tts_server.py`):
```python
spk2info = torch.load('utils/new_spk2info.pt', map_location='cpu', weights_only=False)
model.frontend.spk2info = spk2info  # 注入到 CosyVoice3 前端
```

### 3. TTS: generator 中 return 值丢失

**现象**: 合成成功但 HTTP 返回 500 `Synthesis failed`。

**原因**: `synthesize()` 是 generator 函数（含 `yield`），非流式分支中的 `return chunk[...]` 会触发 `StopIteration` 而不是返回值给调用方。

**修复**: 统一使用 `yield` 输出数据，非流式时 `yield` 后 `return` 终止迭代。

### 4. S2S WebSocket: Opus 编码格式

**注意**: 客户端发送的 Opus 数据必须是 `sphn.OpusStreamWriter.append_pcm()` 返回的完整 Ogg Opus 帧，**不能按固定字节切片**。服务端的 `sphn.OpusStreamReader.append_bytes()` 解析 Ogg 容器头，截断帧会报 `unexpected ogg capture pattern` 错误。

正确做法：每次 `append_pcm(frame)` → 得到完整 opus bytes → 前缀 `0x01` 发送。
